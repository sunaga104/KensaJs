...
export function callSubTitle(
  msg: string,
  paragraph: number,
  index?: number
): void {
  const spaces = ' '.repeat(paragraph);
  console.log(spaces, index ? `${index}.` : '', logStyle(msg).bold().build());
}
...
export function successLog(title: string, paragraph: number) {
  const spaces = ' '.repeat(paragraph + 1);
  console.log(spaces, logStyle('✓').green().bold().build(), '', title);
}
...
export function displayStartMsg() {
  space();
